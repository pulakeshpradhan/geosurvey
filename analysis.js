...
const Analysis = (() => {
  ...
  function run() {
    ...
    if (!worker) { worker = new Worker('stats-worker.js?v=1.9.0'); worker.onmessage = onResult; worker.onerror = e => { running = false; setStatus($('#anStatus'), 'Analysis error: ' + e.message, 'err'); }; }
    running = true;
