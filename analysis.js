...
const Analysis = (() => {
  ...
  function run() {
    ...
    if (!worker) { worker = new Worker('stats-worker.js'); worker.onmessage = onResult; worker.onerror = e => { running = false; setStatus($('#anStatus'), 'Analysis error: ' + e.message, 'err'); }; }
    running = true;
  ...
  function render(r) {
    ...
    if (t.tGender) inf += `<h3 class="an-h3">Independent-samples t-test (Welch): Well-being by gender of head</h3>` + table(['Group', 'n', 'Mean'], [['Male', t.tGender.nA, f(t.tGender.meanA)], ['Female', t.tGender.nB, f(t.tGender.meanB)]]) + note(`t(${f(t.tGender.df, 1)}) = ${f(t.tGender.t)}, p ${pEq(t.tGender.p)}, Cohen's d = ${f(t.tGender.d)} → ${t.tGender.p < 0.05 ? 'significant difference' : 'no significant difference'}.`);
    ...
    inf += an(t.anovaHouse, 'Well-being by house type') + an(t.anovaIncome, 'Economic security by income bracket') + an(t.anovaEdu, 'Economic security by education');
    ...
    if (r.regression) { const g = r.regression; inf += `<h3 class="an-h3">Multiple regression: Well-being on constructs and wealth indicators</h3>` + table(['Predictor', 'B', 'SE', 'β', 't', 'p'], g.coefs.map(c => [esc(c.name), f(c.b, 3), f(c.se, 3), c.beta == null ? '—' : f(c.beta, 3), f(c.t), `<span class="${sigCls(c.p)}">${pf(c.p)}${stars(c.p)}</span>`])) + note(`R² = ${f(g.r2, 3)}, adjusted R² = ${f(g.adjR2, 3)}, F(${g.df1}, ${g.df2}) = ${f(g.F)}, p ${pEq(g.pF)}, n = ${g.n}.`); }
    ...
    if (r.pls) {
      ...
      html += `<h3 class="an-h3">Mediation analysis</h3>` + table(['Indirect path', 'Indirect β', '95% CI', 'p', 'Direct β', 'p', 'Total', 'VAF', 'Conclusion'], p.mediation.map(m => [`${esc(cname(m.a))} → ${esc(cname(m.m))} → ${esc(cname(m.b))}`, f(m.indirect.est, 3), `[${f(m.indirect.lo, 3)}, ${f(m.indirect.hi, 3)}]`, `<span class="${sigCls(m.indirect.p)}">${pf(m.indirect.p)}</span>`, f(m.direct.est, 3), pf(m.direct.p), f(m.total, 3), Number.isFinite(m.vaf) ? (m.vaf * 100).toFixed(0) + '%' : '—', m.type])) + note('Indirect effects are tested with bias-free percentile bootstrap CIs; VAF = indirect / total (20–80% ≈ partial, > 80% ≈ full mediation).');
  ...
  function report(r) {
    ...
    if (t.tGender) bits.push(`well-being ${t.tGender.p < 0.05 ? 'differs' : 'does not differ'} significantly between male- and female-headed households (t = ${f(t.tGender.t)}, p ${pEq(t.tGender.p)}, d = ${f(t.tGender.d)})`);
    if (t.anovaHouse) bits.push(`well-being ${t.anovaHouse.p < 0.05 ? 'varies' : 'does not vary'} by house type (F = ${f(t.anovaHouse.F)}, p ${pEq(t.anovaHouse.p)}, η² = ${f(t.anovaHouse.eta2, 3)})`);
    if (t.anovaIncome) bits.push(`economic security ${t.anovaIncome.p < 0.05 ? 'rises' : 'does not differ'} across income brackets (F = ${f(t.anovaIncome.F)}, p ${pEq(t.anovaIncome.p)})`);
    ...
    if (r.regression) { const g = r.regression; const sig = g.coefs.slice(1).filter(c => c.p < 0.05).sort((a, b) => Math.abs(b.beta) - Math.abs(a.beta)); P.push(`<p><b>Regression.</b> The predictors explain ${(g.adjR2 * 100).toFixed(0)}% of the variance in well-being (adj. R² = ${f(g.adjR2, 3)}, F = ${f(g.F)}, p ${pEq(g.pF)}). ${sig.length ? 'Significant predictors: ' + sig.map(c => `${c.name} (β = ${f(c.beta)}, p ${pEq(c.p)})`).join(', ') + '.' : 'No predictor reached significance.'}</p>`); }
    ...
    if (r.pls) {
      ...
      P.push(`<p><b>Mediation.</b> ${p.mediation.map(m => `${cname(m.a)} → ${cname(m.m)} → ${cname(m.b)}: indirect β = ${f(m.indirect.est, 3)} (95% CI ${f(m.indirect.lo, 3)} to ${f(m.indirect.hi, 3)}), direct β = ${f(m.direct.est, 3)} — ${m.type.toLowerCase()}`).join('; ')}.</p>`);
